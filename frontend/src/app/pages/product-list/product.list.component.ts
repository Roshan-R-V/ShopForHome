...
export class ProductListComponent implements OnInit, OnDestroy {
  ...
  loading = false;
  file: File = null;
  searchText = '';  // Declare searchText variable
  ...
  remove(productInfos: ProductInfo[], productInfo) {
    this.productService.delelte(productInfo).subscribe(_ => {
      productInfos = productInfos.filter(e => productInfo !== e.productId);
    });
  }
  ...
  search() {
    console.log('Search triggered with term: ', this.searchText);
  }
}
